    ...
    <div>
      <div className="flex flex-col md:flex-row gap-8">
        <div className="flex-1 order-2 md:order-1">
          <div className="bg-slate-100 dark:bg-slate-900 rounded-lg p-6 h-96 flex flex-col justify-end items-center">
            <div className="w-full max-w-xs flex flex-col-reverse items-center">
              <AnimatePresence>
                {stack.map((value, index) => (
                  <motion.div
                    ...
                    animate={{
                      opacity: 1,
                      x: 0,
                      scale: peekActive && index === stack.length - 1 ? 1.05 : 1,
                      boxShadow:
        ...
        <div className="flex-1 order-1 md:order-2">
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Stack Operations</h3>
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button onClick={push} disabled={isAnimating || inputValue === ""}>
                  <Plus className="mr-2 h-4 w-4" /> Push
                </Button>
              </div>

              <div className="flex gap-2">
                <Button onClick={pop} disabled={isAnimating || stack.length === 0} variant="outline" className="flex-1">
              ...
              <Button onClick={resetStack} disabled={isAnimating} variant="outline" className="w-full">
                <RotateCcw className="mr-2 h-4 w-4" /> Reset
              </Button>
            </div>

            <div className="mt-8">
              <h4 className="font-medium text-slate-800 dark:text-white mb-2">About Stacks</h4>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                <div className="bg-slate-100 dark:bg-slate-900 p-3 rounded-md">
                  ...
                  <p className="text-xl font-bold text-slate-800 dark:text-white">{stack.length}</p>
                </div>
                <div className="bg-slate-100 dark:bg-slate-900 p-3 rounded-md">
                  <p className="text-sm text-slate-500 dark:text-slate-400">Top Element</p>
