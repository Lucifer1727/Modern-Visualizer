...
export default function ArrayVisualizer() {
  ...
  const bubbleSort = async () => {
    ...
    for (let i = 0; i < n; i++) {
      setSortExplanation(
        `Pass ${
          i + 1
        }: Moving the largest unsorted element to its correct position.`
      );
      ...
      for (let j = 0; j < n - i - 1; j++) {
        ...
        setSortExplanation(
          `Comparing ${newArray[j]} and ${newArray[j + 1]}...`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, 1000 - animationSpeed * 8)
        );

        if (newArray[j] > newArray[j + 1]) {
          // Explain the swap
          setSortExplanation(
            `${newArray[j]} > ${newArray[j + 1]}, so we'll swap them.`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 1000 - animationSpeed * 8)
          );
          ...
          setSortExplanation(
            `${newArray[j]} <= ${newArray[j + 1]}, no swap needed.`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 1000 - animationSpeed * 8)
          );
      ...
      setSortExplanation(
        `Pass ${i + 1} complete: The largest ${i + 1} element${
          i > 0 ? "s are" : " is"
        } now at the end.`
      );
      await new Promise((resolve) => setTimeout(resolve, 1000));
  ...
  const quickSort = async () => {
    ...
    // Initial explanation
    setSortExplanation(
      "Starting Quick Sort: We'll pick a pivot element and partition the array around it."
    );
    await new Promise((resolve) => setTimeout(resolve, 1500));
    ...
    const partition = async (arr: number[], low: number, high: number) => {
      ...
      for (let j = low; j < high; j++) {
        ...
        if (arr[j] < pivot) {
          i++;
          setSortExplanation(
            `${arr[j]} < ${pivot}, so we'll move it to the left side by swapping with position ${i}.`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 1000 - animationSpeed * 8)
          );
          ...
          setSortExplanation(
            `${arr[j]} >= ${pivot}, so we'll leave it on the right side.`
          );
          await new Promise((resolve) =>
            setTimeout(resolve, 1000 - animationSpeed * 8)
          );
      ...
      setSortExplanation(
        `Moving pivot ${pivot} to its correct position between smaller and larger elements.`
      );
      await new Promise((resolve) =>
        setTimeout(resolve, 1000 - animationSpeed * 8)
      );
    ...
    const sort = async (arr: number[], low: number, high: number) => {
      if (low < high) {
        ...
        setSortExplanation(
          `Now sorting the left subarray (elements < ${arr[pi]}).`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, 1000 - animationSpeed * 8)
        );
        ...
        setSortExplanation(
          `Now sorting the right subarray (elements > ${arr[pi]}).`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, 1000 - animationSpeed * 8)
        );
